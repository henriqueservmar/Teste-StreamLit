...
def authenticate():
    ...
    usernames = ["henrique.canhadas", "vitor.lucas", "aa"]
    ...
    if st.session_state["authentication_status"] is True:
        # Se o usuário já estiver autenticado, não exibir campos de entrada e botão de login
        return
    ...
    if st.button("Login"):
        if username_input in usernames:
            ...
            if verify_password(password_input, hashed_passwords[index]):
                ...
                st.write("Bem-vindo! Você está logado com sucesso.")
                return
